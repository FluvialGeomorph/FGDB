...
| Survey/Acquisition Event | SOSA `ObservationCollection` | Candidate alignment for the real-world acquisition/observation event. It is not the processing activity or its results. |
| Analysis/Processing Run | PROV `Activity` | Strong conceptual alignment for a particular execution of a versioned method with parameters, inputs, outputs, agent, and processing time. |
| Source Dataset | DCAT `Dataset`; PROV `Entity` | Dataset produced by or representing an acquisition and consumed by one or more processing runs. |
| Derived Dataset | DCAT `Dataset`; PROV `Entity` | Dataset generated by a processing run; distinguish dataset identity from its individual features and GIS representation. |
...
3. Can a query retrieve all Flowlines, Cross Sections, terrain assumptions,
   source datasets, methods, parameters, and software versions for one
   reach-acquisition and processing run?
...
7. Can acquisition, source dataset, processing run, and derived-result identity
   be traversed without overloading a single Survey Event record?

...
FGDB integrity benefits from a reviewed Network Observation that makes the
...
1. Use one versioned Network Observation contract for all producers and the
   FGDB loader, with controlled evidence/provenance classes at least for:
   `SOURCE_NETWORK_RETAINED`, `RECONSTRUCTED_FROM_REACH_FLOWLINES`, and
   `NETWORK_NOT_RETAINED`.
2. New producer workflows should create and review a Network Observation as
...
3. Automation must not infer project purpose silently. The user still confirms
   the intended scope/outlet, retained paths, Stream/Reach classification, and
...
5. When the original network is missing but applicable Reach Flowlines survive,
   ...
   reviewed Network Observation. It must be labeled
...
6. Reconstruction may copy/assemble Flowline paths, split them deterministically
   ...
   and partial scope coverage require explicit resolution or qualified status.
...
9. FGDB schema and loader development may proceed once the common contract,
   ...
   publication rules may require a reviewed source Network Observation.

## Recommended sequence

1. Finalize the Network Observation exchange contract, evidence classes,
...
3. Implement `fluvgeo` network validators and a reviewed
   ...
   migration workflow.
4. Implement the FGDB package validator and loader against the same contract,
   initially supporting retained and reviewed-reconstructed packages plus an
...
- New Shiny and desktop work converge on the same open-source scientific
  functions and stronger package contract.
...
Network Observation package contract before specifying either producer tools
or the FGDB loader. That contract must resolve physical field types and
domains, segment/topology grain, offline identity and reconciliation,
Flowline-reconstruction lineage, validation outcomes, and package-manifest
representation. Tool specifications then bind to that shared contract rather
than defining independent schemas.
